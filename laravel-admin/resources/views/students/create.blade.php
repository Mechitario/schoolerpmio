...
<div class="max-w-xl">
    ...
    <div class="bg-white dark:bg-slate-800/80 border border-gray-200 dark:border-slate-700 rounded-xl shadow-sm dark:shadow-none p-6">
        <h2 class="text-gray-900 dark:text-white mb-4">Add New Student</h2>
        ...
        <form method="POST" action="{{ route('admin.students.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="class_name" id="class_name" required
                    ...
                    @foreach(range(1, 12) as $n)
                        <option value="{{ $n }}" {{ old('class_name') === (string)$n ? 'selected' : '' }}>Class {{ $n }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="year" id="year" required
                    ...
                    @foreach(range((int) date('Y'), (int) date('Y') - 15) as $y)
                        <option value="{{ $y }}" {{ old('year', date('Y')) === (string)$y ? 'selected' : '' }}>{{ $y }}</option>
                    @endforeach
                </select>
                <p class="mt-1 text-xs text-gray-500 dark:text-slate-500">Year/batch the student belongs to (e.g. 2024).</p>
            </div>
            <div>
            ...
            <div>
                ...
                <select name="parent_id" id="parent_id"
                    ...
                    <option value="{{ $p->id }}" {{ old('parent_id') == $p->id ? 'selected' : '' }}>{{ $p->name }}{{ $p->phone ? ' — ' . $p->phone : '' }}</option>
                    @endforeach
                </select>
            </div>
    ...
    <div class="mt-8 p-4 rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50">
        <h3 class="text-gray-700 dark:text-slate-300 mb-2">Bulk upload</h3>
        <p class="text-sm text-gray-500 dark:text-slate-400 mb-3">Add many students at once using an Excel file (save as CSV). Download the template, fill it, and upload.</p>
        <a href="{{ route('admin.students.import') }}" class="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-cyan-600 dark:text-cyan-400 hover:text-cyan-700 dark:hover:text-cyan-300 border border-cyan-500/30 rounded-lg hover:bg-cyan-500/10 transition-colors">
