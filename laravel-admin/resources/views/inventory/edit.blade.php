...
@section('breadcrumb', 'Edit Inventory Item')
...
<div class="max-w-xl">
    ...
    <div class="bg-white dark:bg-slate-800/80 border border-gray-200 dark:border-slate-700 rounded-xl shadow-sm dark:shadow-none p-6">
        <h2 class="text-gray-900 dark:text-white mb-4">Edit Item / Field</h2>
        ...
        <form method="POST" action="{{ route('admin.inventory.update', $inventory) }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $inventory->name) }}" required autofocus placeholder="e.g. Books, Dress"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="total_lot" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Total lot <span class="text-rose-500">*</span></label>
                    <input type="number" name="total_lot" id="total_lot" value="{{ old('total_lot', $inventory->total_lot) }}" min="0" step="1" required
                ...
                <div>
                    <label for="sold_lot" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Sold lot <span class="text-rose-500">*</span></label>
                    <input type="number" name="sold_lot" id="sold_lot" value="{{ old('sold_lot', $inventory->sold_lot) }}" min="0" step="1" required
                           class="w-full px-4 py-2.5 bg-gray-50 dark:bg-slate-700/50 border border-gray-300 dark:border-slate-600 rounded-lg text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-colors">
                </div>
            </div>
            <p class="text-sm text-gray-500 dark:text-slate-400">Remaining lot = Total lot − Sold lot = <strong>{{ number_format($inventory->remaining_lot) }}</strong></p>
